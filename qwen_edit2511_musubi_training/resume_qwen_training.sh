...
# ==============================================================================
echo -e "${PURPLE}################################################################${NC}"
echo -e "${CYAN}               QWEN EDIT-2511 - RESUME TRAINING SCRIPT            ${NC}"
echo -e "${PURPLE}################################################################${NC}"
echo ""
...
# Main
########################################
# Allow passing a specific config, otherwise default to the Qwen one
CONFIG_FILE="${1:-qwen_musubi_config.sh}"
...
if [ -n "$RESUME_CHECKPOINT" ]; then
    ...
    if [ "${FP8_VL:-0}" = "1" ]; then COMMON_FLAGS+=("--fp8_vl"); fi
    ...
    print_info "Launching Resume Training..."
    cd "$REPO_DIR"
    ...
    print_info "No checkpoint selected. Launching fresh training via main script..."
    bash setup_and_train_qwen_image.sh
fi
