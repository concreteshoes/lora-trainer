...
# Main
########################################
CONFIG_FILE="${1:-flux2_musubi_config.sh}"
...
# 2. Reconstruct paths (Ensuring alignment with main script)
...
if [ -n "$RESUME_CHECKPOINT" ]; then
    ...
    # Execution
    ########################################
    print_info "Launching resume command..."
    cd "$REPO_DIR"
    ...
    # Call your standard Flux training setup script
    bash setup_and_train_flux2.sh
fi
