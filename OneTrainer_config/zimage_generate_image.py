...
def remap_onetrainer_lora_keys(state_dict):
    """Remap OneTrainer LoRA keys to Musubi format.
    
    OneTrainer: transformer.layers.0.attention.to_q.lora_down.weight
    Musubi:     lora_unet_transformer_layers_0_attention_to_q.lora_down.weight
    """
    # Detect if this is an OneTrainer LoRA
    sample_key = next(iter(state_dict))
    if not sample_key.startswith("transformer."):
        return state_dict  # Already in Musubi format, no remapping needed

    logger.info(
        "Detected OneTrainer LoRA format. Remapping keys to Musubi format...")
    new_sd = {}
    for key, value in state_dict.items():
        # Split on the last dot-separated suffix (.lora_down.weight, .lora_up.weight, .alpha)
        if ".lora_down." in key or ".lora_up." in key:
            parts = key.rsplit(".lora_", 1)
            module_path = parts[0]  # e.g. transformer.layers.0.attention.to_q
            suffix = ".lora_" + parts[1]  # e.g. .lora_down.weight
        elif key.endswith(".alpha"):
            module_path = key[:-6]  # remove .alpha
            suffix = ".alpha"
        else:
            logger.warning(f"Unexpected key format during remapping: {key}")
            new_sd[key] = value
            continue
    ...
    logger.info(
        f"Remapped {len(new_sd)} keys from OneTrainer to Musubi format")
    return new_sd
...
def load_dit_model(
    ...
    if not args.lycoris and args.lora_weight is not None and len(args.lora_weight) > 0:
        ...
        for lora_weight in args.lora_weight:
            ...
            lora_sd = remap_onetrainer_lora_keys(lora_sd)
