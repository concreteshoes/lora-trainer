...
DEFAULT_GEMMA_PATH = "/workspace/models/ltx/gemma_3_12B_it_fp8_e4m3fn.safetensors"
DEFAULT_LTX2_CHECKPOINT = "/workspace/models/ltx/ltx-2.3-22b-dev.safetensors"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--ltx2_checkpoint",
        type=str,
        default=DEFAULT_LTX2_CHECKPOINT,
    ...
    # -- Gemma text encoder --
    parser.add_argument("--gemma_root", type=str, default=DEFAULT_GEMMA_PATH,
...
def _merge_lora_weights(
    ...
    for idx, path in enumerate(weights):
        multiplier = multipliers[idx] if multipliers and len(
            multipliers) > idx else 1.0
        logger.info("Merging LoRA: %s (multiplier=%.3f)", path, multiplier)
        lora_sd = load_file(path)

        # --- NEW CODE START: ComfyUI / diffusion-pipe LoRA conversion ---
        try:
            from musubi_tuner.ltx_2.convert_lora_to_comfy import (
                convert_lora_from_comfy_state_dict,
                is_comfy_lora_state_dict,
            )
            if is_comfy_lora_state_dict(lora_sd):
                logger.info(
                    "Detected ComfyUI format LoRA. Converting state dict on the fly...")
                lora_sd = convert_lora_from_comfy_state_dict(lora_sd)
        except Exception as e:
            logger.warning(
                "Failed to check or convert ComfyUI LoRA format: %s", e)
        # --- NEW CODE END ---

        # Auto-detect connector LoRA and attach connectors to wrapper for merge
        has_connector_lora = any(
            "embeddings_connector" in k for k in lora_sd.keys())
